...
def _build_update_payload(
    ...
    if settings.classify_title and classification.title:
        if settings.mode == "overwrite" or _is_autogenerated_title(
...
def _is_autogenerated_title(title: str, original_file_name: str = "") -> bool:
    if not title.strip():
        return True

    lower = title.lower()

    # Check for file extensions and common scanner/camera prefixes
    autogen_patterns = [
        ".pdf",
        ".jpg",
        ".jpeg",
    ...
    if any(p in lower for p in autogen_patterns):
        return True

    # Check for common generic titles
    generic_titles = ["document", "untitled", "attachment", "download"]
    if lower.strip() in generic_titles:
        return True

    # Check if title matches the original filename (with or without extension)
    if original_file_name:
        ...
        fname_no_ext = re.sub(r"\.[^.]+$", "", fname)
        if fname_no_ext.lower() == lower:
            return True
        # Compare alphanumeric-only to handle separator normalization
        # (e.g. Paperless converting dashes to dots or vice versa)
        title_alnum = re.sub(r"[^a-z0-9]", "", lower)
        fname_alnum = re.sub(r"[^a-z0-9]", "", fname.lower())
        fname_no_ext_alnum = re.sub(r"[^a-z0-9]", "", fname_no_ext.lower())
        if title_alnum and (title_alnum == fname_alnum or title_alnum == fname_no_ext_alnum):
            return True
